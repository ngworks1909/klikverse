...
export class FastLudoGame {
    ...
    public startGame(){
        ...
        socketManager.broadcast(this.roomId, 'START_GAME', this.players.toString());
    ...
    public addPlayer(userId: string){
        ...
        this.playerPositions[userId] = [1, 1, 1, 1];
    }
        ...
        for (const [playerId, pieces] of Object.entries(this.playerPositions)) {
            if (playerId !== currentPlayerId) {
                for (let i = 0; i < pieces.length; i++) {
                    ...
                    if (opponentPiecePosition === normalizedPosition && !this.isSafePosition(normalizedPosition)) {
                        // Kill the opponent's piece
                        pieces[i] = 1; // Send piece back to home
    ...
    public rollDice(roomId: string, playerId: string){
        ...
        const message = JSON.stringify({diceValue: steps})
